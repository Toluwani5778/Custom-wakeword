...
def torchaudio_info(path):
    ...
    return type(
        ...
        (),
        {
            "num_frames": meta.duration_seconds * meta.sample_rate,
...
if __name__ == '__main__':
    ...
    from piper_sample_generator.__main__ import (
    generate_samples,
    generate_samples_onnx,
)

    def generate_samples_wrapper(*args, model=None, **kwargs):
        if str(model).endswith(".onnx"):
            return generate_samples_onnx(*args, model=model, **kwargs)
        else:
            return generate_samples(*args, model=model, **kwargs)
    ...
    if args.generate_clips is True:
        ...
        if n_current_samples <= 0.95*config["n_samples"]:
            generate_samples_wrapper(
                ...
                model=config["piper_tts_model_path"],
    ...
    if args.augment_clips is True:
        ...
        missing = any(
            not os.path.exists(os.path.join(feature_save_dir, f))
            for f in required_files
        )

        if missing or args.overwrite:
